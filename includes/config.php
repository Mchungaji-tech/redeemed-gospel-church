...
function rgcConfig(string $key, mixed $default = null): mixed {
  ...
  if ($config === null) {
    $config = [
      ...
      'mail.from_name' => rgcEnv('RGC_MAIL_FROM_NAME', 'Redeemed Gospel Church'),
      'mailbox.key' => rgcEnv('RGC_MAILBOX_KEY', ''),
      'smtp.host' => rgcEnv('RGC_SMTP_HOST', ''),
      'smtp.port' => (int) rgcEnv('RGC_SMTP_PORT', '587'),
      'smtp.username' => rgcEnv('RGC_SMTP_USERNAME', ''),
      'smtp.password' => rgcEnv('RGC_SMTP_PASSWORD', ''),
      'smtp.encryption' => rgcEnv('RGC_SMTP_ENCRYPTION', 'tls'),
      'smtp.timeout' => (int) rgcEnv('RGC_SMTP_TIMEOUT', '15'),
