...
function paypalCreateOrder(string $baseUrl, string $accessToken, array $donation): ?array {
  $ch = curl_init();
  $amount = number_format(($donation['amount_cents'] ?? 0) / 100, 2, '.', '');
  ...
  $body = [
    ...
    'purchase_units' => [[
      ...
      'description' => 'Donation to Redeemed Gospel Church'
    ]]
  ];

  curl_setopt_array($ch, [
    CURLOPT_URL => rtrim($baseUrl, '/') . '/v2/checkout/orders',
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_POST => true,
...
if ($donationId <= 0) {
  paypalFail('Invalid donation ID');
}
...
// Create Order
$order = paypalCreateOrder($baseUrl, $accessToken, $donation);
if (!$order || empty($order['id'])) {
  paypalFail('Failed to create PayPal order', 500);
}
